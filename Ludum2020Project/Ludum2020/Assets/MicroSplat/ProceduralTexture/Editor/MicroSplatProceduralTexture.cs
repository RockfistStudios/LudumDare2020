...
namespace JBooth.MicroSplat
{
   ...
   public class MicroSplatProceduralTexture : FeatureDescriptor
   {
      ...
      void DrawCurveToggle(ref bool toggle, ref AnimationCurve curve, string label, bool endHorizontal = true)
      {
      ...
      int DrawLayer(MicroSplatProceduralTextureConfig.Layer layer, int layerIndex, Material mat, bool showBiome, bool showBiome2, bool showCav, bool showErosion)
      {
         int ret = 0;
         ...
         using (new EditorGUILayout.VerticalScope("box"))
         {
            ...
            DrawCurveToggle(ref layer.heightActive, ref layer.heightCurve, "Height");
            DrawCurveToggle(ref layer.slopeActive, ref layer.slopeCurve, "Slope", false);
            ...
            if (showCav)
            {
               DrawCurveToggle(ref layer.cavityMapActive, ref layer.cavityMapCurve, "Cavity");
            }
            if (showErosion)
            {
               DrawCurveToggle(ref layer.erosionMapActive, ref layer.erosionMapCurve, "Erosion");
            }
      ...
      public override void DrawShaderGUI(MicroSplatShaderGUI shaderGUI, MicroSplatKeywords keywords, Material mat, MaterialEditor materialEditor, MaterialProperty[] props)
      {
         if ((proceduralTexture || heightGradients || heightHSV) && MicroSplatUtilities.DrawRollup("Procedural Texture"))
         {
            ...
            if (proceduralTexture && mat.HasProperty("_ProcTexNoise"))
            {
               ...
               {
                  int reorder = DrawLayer(cfg.layers[i], i, mat, showBiomes, showBiomes2, showCav, showErosion);
                  if (reorder == 3)
                  {
                     var cl = cfg.layers [i];
                     var nl = cl.Copy ();
