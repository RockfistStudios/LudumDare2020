   ...
   weights = half3(w1, w2, w3);
                     ...
                     float2 dx, float2 dy, float3 wdx, float3 wdy)
      {
         ...
         half4 params3 = half4(1,1,1,1);
            ...
            half4 noise = MICROSPLAT_SAMPLE_TEX2D_SAMPLER_LOD(_ProcTexNoise, _Diffuse, float2(noiseUV * params0.r + params0.a), lod);
         ...
         texIndex = params1.g;
         return saturate(h0 * s0 * c0 * e0 * params1.x * bWeight * bWeight2);
      }
      ...
      void ProceduralSetup(Input i, float3 worldPos, float worldHeight, float3 worldNormal, float3 up, inout half4 weights, float2 uv, inout Config config, float2 dx, float2 dy, float3 wdx, float3 wdy)
      {
         #if _PROCEDURALBLENDSPLATS
         ...
         // when blending is enabled, we need to resort the weights/indexes between a list of 8
            ...
            #if _MAX2LAYER
               mergeWeights.zw = 0;
               mergeWeights.xy *= (1.0 / (mergeWeights.x + mergeWeights.y));
            #endif
            #if _MAX3LAYER
               mergeWeights.w = 0;
               mergeWeights.xyz *= (1.0 / (mergeWeights.x + mergeWeights.y + mergeWeights.z));
            #endif
      ...
      void ProceduralGradients(Input i, inout RawSamples samples, Config c, float worldHeight, float3 worldNormal)
      {
         float offset = 0.5 / 32;
         ...
         #if _PCHEIGHTGRADIENT
            samples.albedo0.rgb *= UNITY_SAMPLE_TEX2D_SAMPLER(_PCHeightGradients, _Diffuse, float2(h, c.uv0.z / 32.0 + offset)) * 2;
            samples.albedo1.rgb *= UNITY_SAMPLE_TEX2D_SAMPLER(_PCHeightGradients, _Diffuse, float2(h, c.uv1.z / 32.0 + offset)) * 2;
            
            #if !_MAX2LAYER
            samples.albedo2.rgb *= UNITY_SAMPLE_TEX2D_SAMPLER(_PCHeightGradients, _Diffuse, float2(h, c.uv2.z / 32.0 + offset)) * 2;
            #endif
            
            #if !_MAX3LAYER && !_MAX2LAYER
            samples.albedo3.rgb *= UNITY_SAMPLE_TEX2D_SAMPLER(_PCHeightGradients, _Diffuse, float2(h, c.uv3.z / 32.0 + offset)) * 2;
            #endif
         #endif
         
         #if _PCSLOPEGRADIENT
            samples.albedo0.rgb *= UNITY_SAMPLE_TEX2D_SAMPLER(_PCSlopeGradients, _Diffuse, float2(slope0, c.uv0.z / 32.0 + offset)) * 2;
            samples.albedo1.rgb *= UNITY_SAMPLE_TEX2D_SAMPLER(_PCSlopeGradients, _Diffuse, float2(slope1, c.uv1.z / 32.0 + offset)) * 2;
            
            #if !_MAX2LAYER
            samples.albedo2.rgb *= UNITY_SAMPLE_TEX2D_SAMPLER(_PCSlopeGradients, _Diffuse, float2(slope2, c.uv2.z / 32.0 + offset)) * 2;
            #endif
            
            #if !_MAX3LAYER && !_MAX2LAYER
            samples.albedo3.rgb *= UNITY_SAMPLE_TEX2D_SAMPLER(_PCSlopeGradients, _Diffuse, float2(slope3, c.uv3.z / 32.0 + offset)) * 2;
            #endif
         #endif
            ...
            #if _PCHEIGHTHSV
               samples.albedo0.rgb += UNITY_SAMPLE_TEX2D_SAMPLER(_PCHeightHSV, _Diffuse, float2(h, c.uv0.z / 32.0 + offset)).xyz - 0.5;
               samples.albedo1.rgb += UNITY_SAMPLE_TEX2D_SAMPLER(_PCHeightHSV, _Diffuse, float2(h, c.uv1.z / 32.0 + offset)).xyz - 0.5;
            
               #if !_MAX2LAYER
               samples.albedo2.rgb += UNITY_SAMPLE_TEX2D_SAMPLER(_PCHeightHSV, _Diffuse, float2(h, c.uv2.z / 32.0 + offset)).xyz - 0.5;
               #endif
            
               #if !_MAX3LAYER && !_MAX2LAYER
               samples.albedo3.rgb += UNITY_SAMPLE_TEX2D_SAMPLER(_PCHeightHSV, _Diffuse, float2(h, c.uv3.z / 32.0 + offset)).xyz - 0.5;
               #endif
            #endif

            #if _PCSLOPEHSV
               samples.albedo0.rgb += UNITY_SAMPLE_TEX2D_SAMPLER(_PCSlopeHSV, _Diffuse, float2(slope0, c.uv0.z / 32.0 + offset)).xyz - 0.5;
               samples.albedo1.rgb += UNITY_SAMPLE_TEX2D_SAMPLER(_PCSlopeHSV, _Diffuse, float2(slope1, c.uv1.z / 32.0 + offset)).xyz - 0.5;
            
               #if !_MAX2LAYER
               samples.albedo2.rgb += UNITY_SAMPLE_TEX2D_SAMPLER(_PCSlopeHSV, _Diffuse, float2(slope2, c.uv2.z / 32.0 + offset)).xyz - 0.5;
               #endif
            
               #if !_MAX3LAYER && !_MAX2LAYER
               samples.albedo3.rgb += UNITY_SAMPLE_TEX2D_SAMPLER(_PCSlopeHSV, _Diffuse, float2(slope3, c.uv3.z / 32.0 + offset)).xyz - 0.5;
               #endif
            #endif
...
UNITY_INSTANCING_BUFFER_END(Terrain)
      ...
      MicroSplatLayer Sample(Input i, half4 weights, inout Config config, float camDist, float3 worldNormalVertex)
      {
         ...
         #if _TRAXSINGLE || _TRAXARRAY || _SNOWFOOTSTEPS
...
ENDCG
   ...
   Dependency "BaseMapGenShader" = "/Example_Base1584205534"
   Dependency "BaseMapShader" = "Hidden/MicroSplat/Example_Base1584205534"
